...
if [[ -n "$S3_BUCKET" ]]; then
  ...
  for f in "$OUT_DIR"/*.aab; do
    [[ -f "$f" ]] || continue
    aws s3 cp "$f" "s3://${S3_BUCKET}/${S3_PREFIX:-bundles}/$JOB_NAME/$BUILD_NUMBER/$(basename "$f")"
  done
  shopt -u nullglob
  ...
  : >"$S3_TSV"
  {
    ...
    for f in "$OUT_DIR"/*.aab; do
      [[ -f "$f" ]] || continue
      fname="$(basename "$f")"
      url=$(aws s3 presign "s3://${S3_BUCKET}/${S3_PREFIX:-bundles}/${JOB_NAME}/${BUILD_NUMBER}/${fname}" \
