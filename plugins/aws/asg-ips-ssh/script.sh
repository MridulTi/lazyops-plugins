...
#
# Env: ASG_FILE, KEY_DIRECTORY, CYTOOL_RECONNECT_ID (32-hex Cortex distribution ID for cytool reconnect force)
#
...
CYTOOL_RECONNECT_ID="${CYTOOL_RECONNECT_ID:-}"
...
run_remote_cytool() {
  local ip="$1"
  if [[ -z "$CYTOOL_RECONNECT_ID" ]]; then
    echo "ERROR: Set CYTOOL_RECONNECT_ID env var (32-hex Cortex distribution ID)" >&2
    return 1
  fi
