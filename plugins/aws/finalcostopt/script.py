...
def get_tag_value(tags, key):
    return next((tag['Value'] for tag in tags if tag['Key'] == key), 'N/A')
    ...
    for instance in reservation['Instances']:
        if instance['State']['Name'] == 'stopped':
           stopped_instances.append({
             ...
             'Techteam': next((tag['Value'] for tag in instance.get('Tags', []) if tag['Key'] == 'Techteam'), None),
             'Project': get_tag_value(instance.get('Tags', []), 'Project')
           })
...
for volume in response['Volumes']:
    ...
    volume_info_list.append({
        ...
        'Project': get_tag_value(volume.get('Tags', []), 'Project')
    })
...
for ASG in response['AutoScalingGroups']:
     ...
     ASGs.append({
         ...
         'Project': get_tag_value(ASG.get('Tags', []), 'Project')
...
for snapshot in snapshot_response['Snapshots']:
    if snapshot['VolumeId'] not in volumes:
        ...
        snapshots.append({
            ...
            'TechTeam': techtag,
            'Project': get_tag_value(snapshot.get('Tags', []), 'Project')
        })
...
for image in custom_images['Images']:
    if image['ImageId'] not in used_amis:
       ...
       custom_amis_list.append({
         ...
         'TechTeam': techtag,
         'Project': get_tag_value(image.get('Tags', []), 'Project')
       })
...
for SecGrp in response['SecurityGroups']:
    if SecGrp['GroupId'] not in sec_groups_in_use:
       ...
       all_sec_groups.append({
          ...
          'TechTeam': techtag,
          'Project': get_tag_value(SecGrp.get('Tags', []), 'Project')
       })
...
for VPC in vpcs:
    ...
    if len(enis) == 0:
        ...
        unusedvpc.append({
            ...
            'TechTeam': techtag,
            'Project': get_tag_value(VPC.get('Tags', []), 'Project')
        })
...
for tgunused in response_targets['TargetGroups']:
    ...
    if len(unused) == 0:
        ...
        elbsv2_notinuse.append({
            ...
            'TechTeam': techtag,
            'Project': get_tag_value(tgunused.get('Tags', []), 'Project')
        })
...
for elbtg in response_lbs['LoadBalancers']:
    ...
    if check is True:
        ...
        unusedlb.append({
            ...
            'TechTeam': techtag,
            'Project': get_tag_value(elbtg.get('Tags', []), 'Project')
        })
